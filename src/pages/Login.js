...
import useValidateRegex from '../components/validator';
import { dbActions } from '../components/crud';
import AppContext from '../components/appcontext';

function Login(){
  const [email, checkEmail, validEmail] = useValidateRegex(null, /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/)
  const [password, checkPassword, validPassword] = useValidateRegex(null, /.{6,}/)
  ...
  return(
    <section className="pageFrame">
      <div className="authCard">
        <form className='option1' spellCheck={false} style={{borderColor: '#21e386'}} autoComplete='on'>
          <h1 className="formHeading" style={{color: '#21e386'}}>Login</h1>
          <div className="inputFrame">
            <label htmlFor="signupEmail">Email</label>
            <input type="email" id="signupEmail" onInput={(e) => checkEmail(e.target.value)}/>
          ...
          <button disabled={!validEmail || !validPassword} style={{backgroundColor: '#21e386'}} className='submitForm' onClick={(e) => submitForm(e)}>
            <span>
        ...
        <button onClick={() => dbActions.signIn('lekanalowooja@yahoo.com', 'Developer@2022')}>Dummy Login</button>
      </div>
    </section>
	)
}
