...
export const Authentication = {
    Required: ({auth, children}) => {
        ...
        if (location.pathname.replaceAll('/','').toLowerCase() === "admin") return <Navigate to="/admin/home" replace />;
        if (!auth) return <Navigate to="/" replace />;
        return <Suspense fallback={<>Loading...</>}>{children}</Suspense>;
    },
    NotRequired: ({auth, children}) => {
        const location = useLocation()
        if (auth) return <Navigate to="/admin/home" state={{ from: location }} replace />;
        return <Suspense fallback={<>Loading...</>}>{children}</Suspense>
    }
}
    ...
    useEffect(() => {
        onAuthStateChanged(auth, (currentUser) => {
            ...
            //* otherwise only permit entry to login, signup and shortlink
                ...
                if(isAuthenticated || isAuthenticated == null) setAuthentication(false)
                setAppData(null);
                setUser(false)
            }
        });
