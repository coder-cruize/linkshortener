import { auth, rtdb, storage } from "../firebaseClient";
import {
...
import { set, update, get, ref, increment } from "firebase/database";
...
export const dbActions = {
    ...
    new Promise((resolve, reject) => {
      try {
        ...
        if (error.code === "auth/email-already-in-use")
          reject("An account already exists with this Email");
      }
    }),
    ...
    new Promise((resolve, reject) => {
      try {
        ...
        if (error.code === "auth/too-many-requests")
          reject("This account has been temporarily disabled. Try again later");
      }
    }),
  signOut: () =>
    new Promise((resolve, reject) => {
  ...
  updateUser: {
    name: (user, data) => new Promise((resolve, reject) => {
      try {
    ...
    photo: (canvas, crop, user) => new Promise((resolve, reject) => {
      const profileRef = storeRef(storage, "users/" + user.uid + "/profile.jpg");
        const handleUpload = (file) => {
          fetch(file)
            .then((res) => res.blob())
            .then((blob) => {
              uploadBytes(profileRef, blob).then(async() => {
                try {
            ...
            .catch((error) => {
              reject(`An error occured while uploading profile. Try Again Later`)
            });
        };
      ...
      getData(`links/${Id}/auth/`)
        .then((data) => {
          if (data.exists() && data.val().live) {
            setData(`links/${Id}/noauth/clicks`, increment(1)).then(() => {
  ...
  getAppData: (uid) =>
    new Promise((resolve, reject) => {
      ...
      getData(`users/${uid}`).then((userLinks) => {
        if (userLinks.val() === null) {
          resolve(dataBoilerPlate);
          return;
        }
        Promise.all(
          Object.keys(userLinks.val()).map(async (link) => {
